...
PacketFence supports a whole lot of different wireless and wired network equipments from various vendors running 
different versions. Since we want to provide the most accurate information and avoid duplication of that same 
information, please refer to our website http://www.packetfence.org/about/supported_switches_and_aps.html
...
Activate the snmp v1 on the switch:
...
============================================================================
There's a lot of different versions of the Catalyst 2960 serie. Some of them
...
In this example, the ACL for registration is 'redirect' and the ACL for registered devices is 'registered'
...
Then add this additionnal configuration on the global level
...
* Select the type to 'Cisco Catalyst 2960 with Web Auth'
* Set 'Portal URL' to 'http://<your_captive_portal_ip>'
* Set the 'Registration' role to 'redirect'
* Set your registered roles to 'registered'
...
PacketFence supports this switch using RADIUS, MAC-Authentication and 802.1Xx.
...
First, configure the global configuration and interfaces using the steps above.

Then create your access list. The one below only allows DHCP and UDP communication with 10.0.0.6

----

access-list IP extended test
 permit UDP any host 10.0.0.6
 permit UDP any any source-port 67
 permit UDP any any destination-port 67
 permit UDP any any source-port 68
 permit UDP any any destination-port 68
 deny UDP any any

----

You also need to add the following to each interface
...
Then in the PacketFence switch configuration, enable `Role by Switch Role` and configure the role(s) to `ip-access-group-in=test;`
...
Setup the radius serveur (@IP of PacketFence) authentication + accounting:
...
----
RS G8052(config)# configure termninal
...
The other way to support Roaming is to enable snmp trap in the AeroHIVE configuration to PacketFence server. PacketFence will recognise the ahConnectionChangeEvent and will change the location of the node in his base.
...
++++++++++++++++++++
* Go in *Configuration -> Authentication -> Captive Web Portals* and create a new portal
* Select Select *Registration Type* = *External Authentication*
* Go in the section *Captive Web Portal Login Page Settings* set the *Login URL* to http://pf_ip/ and *Password Encryption* to *No Encryption*

////

This option is currently broken but will be needed so it redirects the user at the end of the authentication.
* Go in the section * Captive Web Portal Success Page Settings* and check *Redirect to the initially requested page* at the bottom.
...
In the guided configuration you now be able to select your new SSID, the Portal you want to use and the AAA server.
...
This does not cover the configuration of other Anyfi network elements such as the Controller. Please refer to http://www.anyfinetworks.com/resources[Anyfi Networks' website] for relevant documentation.
...
Hostapd must have been compiled with dynamic vlan support and you need to create a file /etc/config/hostapd.vlan that contain:
...
Configure your SSID using uci command:
  ...
  uci add_list wireless.@wifi-iface[0].radius_das_client='192.168.1.5 useStrongerSecret'
...
To configure Hostapd software you can use the same configuration parameters above in the configuration file.
...
CAUTION: There is currently no way to reevaluate the access of the device using PacketFence. This means that there is no real-time action when trying to isolate a device. The RADIUS reauthentication delay will determine the maximum response time of the access reevaluation.

First create a new SSID and then edit its settings.

Configure it as seen in the screenshot below.

image::docs/images/meraki-radius-config.png[scaledwidth="100%",alt="RADIUS configuration"]

NOTE: Make sure to configure your RADIUS server so it's accessible externally under a domain name or a public IP address. Replace `pf.domain.net` by what you will configure. The only port needed is 1812 in UDP. Refer to the Meraki documentation for more details.

Next, just below, configure the options for the users access as shown in the screenshot below.

image::docs/images/meraki-network-config.png[scaledwidth="100%",alt="Network configuration"]

Where 192.168.1.5 is the IP address of your PacketFence server.

NOTE: Make sure you select bridge mode so your users are not NATed through your AP. If you need the devices to be on another VLAN, modify the VLAN tagging of the SSID.

Next go in *Wireless->Configure->Splash page*. Select the *Custom splash URL* and configure the URL pointing to your PacketFence server.

image::docs/images/meraki-splash-config.png[scaledwidth="100%",alt="Splash page configuration"]

NOTE: Even if you enabled the bridge mode on the access point, it will still send a NATed IP address during the initial HTTP request. To workaround this you will need to forward the DHCP traffic of the network your nodes are connected to so PacketFence has the proper IP information of the devices.
...
image::docs/images/ruckus_hotspot_service.png[scaledwidth="100%",alt="Hotsport Service"]

 1 - Name of your Hotspot service
 2 - Login Page: Url of Packetfence portal interface (http://192.168.1.5)
...
image::docs/images/ruckus_create_ssid.png[scaledwidth="100%",alt="Hotsport Service"]
...
* Set the *Redirect URL* to http://192.168.1.5/Xirrus::AP_http
