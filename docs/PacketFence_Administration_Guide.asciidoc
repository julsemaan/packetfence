...
NOTE: Under RHEL 5.x / Centos 5.x, you must exclude packages from PacketFence repository edit /etc/yum.repos.d/PacketFence.repo and add on the section [PacketFence] the line:

  exclude = perl-HTML-FormHandler*
...
The following can be configured by editing the pf.conf file:
   ...
   [gaming_devices_registration]
   category = THE_CATEGORY_YOU_WANT_TO_ASSIGN

Make sure the category you set in the pf.conf file actually exists in PacketFence otherwise you will encounter registration errors. 

Proxy Interception
~~~~~~~~~~~~~~~~~~

To access to the captive portal if a proxy is configured on the pc, you have the possibility to intercept the traffic to the proxy server.
To enable this feature, you have to configure pf.conf in that way:
...
Where server is the fqdn name of the captive portal.

So all the request to a proxy on the port 8080 and 3128 will be intercept by PacketFence and iredirect to the captive portal.
