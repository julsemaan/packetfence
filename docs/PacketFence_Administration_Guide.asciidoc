...
* *Publishable key* is the secret key you got from your Stripe account.
...
PacketFence supports the subscription based billing with Stripe. 
...
You can keep the default configuration, modify the App ID & App Secret (Given by Google on the developper plateform) and Portal URL (https://YOUR_PORTAL_HOSTNAME/oauth2/google).
...
https://YOUR_PORTAL_HOSTNAME/oauth2/twitter
