...
sub class_db_prepare {
    ...
    $class_statements->{'class_add_sql'} = get_db_handle()->prepare(
        qq [ insert into class(vid,description,auto_enable,max_enables,grace_period,window,vclose,priority,url,max_enable_url,redirect_url,button_text,enabled,vlan,target_category) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ]);

    $class_statements->{'class_modify_sql'} = get_db_handle()->prepare(
        qq [ update class set description=?,auto_enable=?,max_enables=?,grace_period=?,window=?,vclose=?,priority=?,url=?,max_enable_url=?,redirect_url=?,button_text=?,enabled=?,vlan=?,target_category=? where vid=? ]);
    ...
    $class_statements->{'class_trappable_sql'} = get_db_handle()->prepare(
        qq [select c.vid,c.description,c.auto_enable,c.max_enables,c.grace_period,c.window,c.vclose,c.priority,c.url,c.max_enable_url,c.redirect_url,c.button_text,c.enabled,c.vlan,c.target_category from class c left join action a on c.vid=a.vid where a.action="trap" ]);
